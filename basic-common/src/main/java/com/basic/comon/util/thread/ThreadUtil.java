package com.basic.comon.util.thread;

import com.basic.comon.util.thread.priormaxpool.EagerThreadPoolExecutor;
import com.basic.comon.util.thread.priormaxpool.TaskQueue;
...
public class ThreadUtil {
     ...
     **/
    private static final ArrayBlockingQueue<Runnable> QUEUE = new ArrayBlockingQueue<>(30);

    /**
     * 2021/5/12 14:14 @railgun threadFactory，定义了线程任务的名称
     **/
    private static final ThreadFactory THREAD_FACTORY = new ThreadFactoryBuilder().setNameFormat("railgun-task-%d").build();
     ...
     **/
    public static ThreadPoolExecutor getPriorMaxPoolThreadPool(boolean timeOut) {
        TaskQueue<Runnable> queue = new TaskQueue<>(100);
        ...
        priorMaxPoolThreadPool.allowCoreThreadTimeOut(timeOut);
     ...
     * railgun
     * 2021/6/21 8:53
     * PS: 创建优先 maxPool 的线程池 自定义队列大小线程池
     **/
    public static ThreadPoolExecutor getPriorMaxPoolThreadPoolWithCustomQueueSize(boolean timeOut, Integer queueSize) {
        TaskQueue<Runnable> queue = new TaskQueue<>(queueSize);
        EagerThreadPoolExecutor priorMaxPoolThreadPool = new EagerThreadPoolExecutor(CORE_POOL_SIZE, MAXIMUM_POOL_SIZE, KEEP_ALIVE_TIME, TimeUnit.SECONDS, queue, THREAD_FACTORY, DEFAULT_HANDLER);
        queue.setExecutor(priorMaxPoolThreadPool);
        priorMaxPoolThreadPool.allowCoreThreadTimeOut(timeOut);
        return priorMaxPoolThreadPool;
